...
public class HistoryController {
     ...
     */
    @GetMapping("/getHistoryAm")
    public ResultVO<Object> getHistoryAm(@RequestParam(value = "limit", defaultValue = "10") int limit) {
        List<LotteryHistory> list = lotteryHistoryMapper.selectLatestList(limit);
    ...
    public ResultVO<Object> getLatestOpenCode() {
        try {
            // 1. 获取数据库最新一条记录
            LotteryHistory dbLatest = lotteryHistoryMapper.selectLatest();
            ...
            String url = "https://macaumarksix.com/api/macaujc2.com";
            ...
            // 检查是否已存在
            LotteryHistory existing = lotteryHistoryMapper.selectByExpect(expect);
            if (existing != null) {
            ...
            lotteryHistoryMapper.insertSelective(history);
    ...
    public ResultVO<Object> getLive() {
        try {
            String url = "https://macaumarksix.com/api/live2";
            HttpResponse<String> response = Unirest.get(url).asString();
    ...
    public ResultVO<Object> getHistoryByExpect(@PathVariable("expect") String expect) {
        try {
            ...
            // 2. 数据库没有，查询外部接口
            String url = "https://history.macaumarksix.com/history/macaujc2/expect/" + expect;
            HttpResponse<String> response = Unirest.get(url).asString();
    ...
    public ResultVO<Object> gethistoryByYear(@PathVariable("year") String year) {
        try {
            String url = "https://history.macaumarksix.com/history/macaujc2/y/" + year;
            HttpResponse<String> response = Unirest.get(url).asString();
            ...
            JSONArray dataArray = jsonObject.getJSONArray("data");
            List<LotteryHistory> historyList = new ArrayList<>();
            ...
            historyList.sort(Comparator.comparing(LotteryHistory::getExpect));

            // Save to database
            int count = 0;
            for (LotteryHistory history : historyList) {
                // Check if exists to avoid duplicates
                LotteryHistory existing = lotteryHistoryMapper.selectByExpect(history.getExpect());
                if (existing == null) {
                    lotteryHistoryMapper.insertSelective(history);
                    count++;
                }
